    ...
    if question == "1":
        ...
        for i in range(-2, 3):
            for j in range(-4,1):
                ...
                lr_kernel = kernelLogRegL2(kernel_fun=logReg.kernel_RBF, lammy=10**j, sigma=10**i)
                lr_kernel.fit(Xtrain, ytrain)

                training_error = np.mean(lr_kernel.predict(Xtrain) != ytrain)
                testing_error = np.mean(lr_kernel.predict(Xtest) != ytest)
                # print(i, j,training_error)

                if min_training > training_error:
                    min_training = training_error
                    best_training_lammy = 10**i
                    best_training_sigma = 10**j
                    best_training_plot = lr_kernel


                if min_testing > testing_error:
                    min_testing = testing_error
                    best_testing_lammy = 10**i
                    best_testing_sigma = 10**j
                    best_testing_plot = lr_kernel
                
               
        print("Training error %.3f, lamda = %.3f, sigma = %.3f " % (min_training, best_training_lammy, best_training_sigma))
        print("Testing error %.3f, lamda = %.3f, sigma = %.3f " % (min_testing, best_testing_lammy, best_testing_sigma))

        utils.plotClassifier(best_training_plot, Xtrain, ytrain)
        utils.savefig("BestTraining.png")

        utils.plotClassifier(best_testing_plot, Xtest, ytest)
        utils.savefig("BestTesting.png")
        ...
        k = 5            # number of PCs
        threshold = 0.2  # threshold for being considered "foreground"
