...
\begin{python}
	...
	return error
\end{python}
\ans{Q2: The error keep decreasing}
\ans{Q3}
\centerfig{.5}{../figs/density2.png}
\ans{Q4:
	\\ n\_cluster = the number of clusters to generate
	\\ init = a method to choose initial center
	\\ n\_init = the number of time to run Kmeans with different initial center
...
\rubric{reasoning:5}
 ...
 \ans{The function assumes that the k is known. The k with smallest error would be the largest value of k.}
 \item Explain why even evaluating the \texttt{error}
 \and function on test data still wouldn't be a suitable approach to choosing $k$.
 \ans{The future data could contain outliers that increase error with given k.}
 ...
 \ans{Either 3 or 4 sould be an ideal k for this data.}
 }}
